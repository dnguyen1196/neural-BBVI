...
def sample_neural_network_from_posterior(posterior_params, n_hiddens_per_layer_list, n_dims_input=1, n_dims_output=1):
    nn_param_list = []
    n_hiddens_per_layer_list = [n_dims_input] + n_hiddens_per_layer_list + [n_dims_output]
    ...
    for i in range(len(n_units_in)):
        ...
        params = posterior_params[i]
        weight_mean = params["mw"]
        weight_log_std = params["sw"]
        bias_mean = params["mb"]
        bias_log_std = params["sb"]
...
def compute_log_q_posterior(nn_params, posterior_params):
    logposterior = 0.0
    for layer_id, layer in enumerate(nn_params):
        mw = posterior_params[layer_id]["mw"]
        sw = np.exp(posterior_params[layer_id]["sw"])
        # Note that posterior params store the log of the standard deviation
        mb = posterior_params[layer_id]["mb"]
        sb = np.exp(posterior_params[layer_id]["sb"])
...
def estimate_ELBO(posterior_params, x_train_N, y_train_N,  num_samples, nn_structure):
    """
    ...
    for num in range(num_samples):
        nn_param_sampled = sample_neural_network_from_posterior(posterior_params, nn_structure)
        log_ll = compute_log_likelihood(nn_param_sampled, x_train_N, y_train_N)
        log_prior = compute_log_prior(nn_param_sampled)
        log_q_posterior = compute_log_q_posterior(nn_param_sampled, posterior_params)
        elbo_sampled_vals[num] = log_ll + log_prior - log_q_posterior

    return np.average(elbo_sampled_vals)


def estimate_VI_loss(posterior_params, x_train_N, y_train_N, num_samples, nn_structure):
    return -estimate_ELBO(posterior_params, x_train_N, y_train_N, num_samples, nn_structure)


def compute_derivative_log_posterior(posterior_params, nn_param_sample):
    derivative_param = copy.deepcopy(posterior_params)

    for layer_id, layer in enumerate(posterior_params):
        w = nn_param_sample[layer_id]["w"]
        b = nn_param_sample[layer_id]["b"]

        mw = np.copy(derivative_param[layer_id]["mw"])
        sw = np.copy(derivative_param[layer_id]["sw"])
        mb = np.copy(derivative_param[layer_id]["mb"])
        sb = np.copy(derivative_param[layer_id]["sb"])
        ...
        derivative_param[layer_id]["mw"]  = (w - mw)/np.exp(2*sw)
        derivative_param[layer_id]["sw"] = -2*sw + np.square(mw-w)/np.exp(2*sw)
        derivative_param[layer_id]["mb"] = (b - mb)/np.exp(2*sb)
        derivative_param[layer_id]["sb"] = -2*sb + np.square(mb-b)/np.exp(2*sb)

    return derivative_param
...
def compute_expected_gradient_ELBO(posterior_params, x_train_N, y_train_N, num_samples, nn_structure):
    """
    ...
    for num in range(num_samples):
        nn_param_sample = sample_neural_network_from_posterior(posterior_params, nn_structure)
        gradient_elbo_sample = compute_derivative_log_posterior(posterior_params, nn_param_sample)

        #log_ll = compute_log_likelihood(nn_param_sample, x_train_N, y_train_N)
        #log_prior = compute_log_prior(nn_param_sample)
        #log_q_posterior = compute_log_q_posterior(nn_param_sample, posterior_params)
        #alpha = log_ll + log_prior - log_q_posterior

        alpha = estimate_ELBO(posterior_params, x_train_N, y_train_N, 10, nn_structure)

        expected_gradient_elbo = add_two_list_dictionaries(expected_gradient_elbo, gradient_elbo_sample, alpha/num_samples)
...
def compute_expected_gradient_VI_loss(posterior_params, x_train_N, y_train_N, num_samples, nn_structure):
    expected_gradient_elbo = compute_expected_gradient_ELBO(posterior_params,x_train_N, y_train_N, num_samples, nn_structure)
    #print("expected gradient elbo", expected_gradient_elbo)
    for layer_id, layer in enumerate(expected_gradient_elbo):
        for key in layer.keys():
            layer[key] = np.multiply(layer[key], -1) # Flip the sign
    return expected_gradient_elbo
...
def learn_neural_approximate_posterior_via_gradient_descent(nn_structure, x_train_N, y_train_N, step_size, num_steps, num_samples):
    ...
    posterior_params = make_posterior_params_as_list_of_dicts(
                            ...
                            mean_weight_fill_func=lambda x : np.random.normal(0, 1, size=x),\
                            log_std_weight_full_func=np.zeros,\
                            mean_bias_fill_func=lambda x: np.random.normal(0, 1, size=x),\
                            log_std_bias_fill_func=np.zeros,)
    ...
    for step in range(num_steps):
        ...
        vi_loss = estimate_VI_loss(posterior_params, x_train_N, y_train_N, 1000, nn_structure)
        vi_losses.append(vi_loss)
        gradient_params = None
        # Compute gradient and check for convergence
        gradient_params = compute_expected_gradient_VI_loss(posterior_params, x_train_N, y_train_N, num_samples, nn_structure)
        #gradient = compute_expected_gradient_ELBO(posterior_params, x_train_N, y_train_N, num_samples, nn_structure)
        gradient_norm = compute_gradient_norm(gradient_params)
        ...
        # v = v - eps * dL/dv
        # Update
        posterior_params =  add_two_list_dictionaries(posterior_params, gradient_params, -1 * step_size(step))

    # Update posterior via gradient descent
    return posterior_params, vi_losses, grad_norm
...
def do_problem_2a(outfolder):
    ...
    num_steps = 2000
    num_samples = 1000
    #step_size = lambda step : 1e-5/(step+1)
    step_size = lambda step : 1e-5
    posterior, vi_losses, grads = learn_neural_approximate_posterior_via_gradient_descent(nn_structure, x_train_N, y_train_N, step_size, num_steps, num_samples)
